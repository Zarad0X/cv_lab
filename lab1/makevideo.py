...
def create_text_frame(text, size, duration_sec, fps):
    ...
    cv2.putText(frame, text, (50, size[1] // 2), 
...
def main(input_path):
    ...
    #  写片头 
    print("写入片头...")
    title_frames = create_text_frame(" Computer Vision HW1 ",
                                      size, duration_sec=2, fps=fps)
    ...
    for img_path in image_files:
        ...
        for _ in range(int(fps * 1.5)):  # 每张图显示1.5秒
            out.write(add_footer_text(img_resized.copy(),
                                       "ZhangHan_3230102282"))
